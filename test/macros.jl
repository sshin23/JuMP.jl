...
@testset "Add vector of variables" begin
    model = Model()
    ...
    @test_throws ErrorException @variable(model, k in MOI.GreaterThan.([3., 2.]))
    @test_throws ErrorException @variable(model, u[1:3] in MOI.GreaterThan.([3., 2.]))
    @test num_variables(model) == 6
end
